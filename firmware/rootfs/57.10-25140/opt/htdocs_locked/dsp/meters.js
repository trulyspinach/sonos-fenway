...
function getDebugMeters( meterIdString )
{
    var http_request = new XMLHttpRequest();
    var debugIndex = meterIdString.indexOf( ".DebugMeter" );
    var blockName = "";
    if (debugIndex > 0) {
        blockName = meterIdString.substring(0, debugIndex);
    }
    else {
        console.log("Debug Meter Index not Found");
        return;
    }
    http_request.open("GET", "getDSP?"+blockName, false);
    http_request.send();
     if (http_request.status === 200) {
      var xml = http_request.responseXML;
        ...
        var debugParams = root.getElementsByTagName("debugParam");
        gMeterIds = new Array();
        ...
        for(var i = 0; i < debugParams.length ; i++) {
            if(debugParams[i].getAttribute("name").includes("DebugMeter")){
      ...
      var xml = http_request.responseXML;
        ...
        for (var i=0; i <blocks.length; i++){
            ...
            if(types[0].textContent == "Meter" ||
                ...
                gIsDebugMeter[optionIndex] = false;
                optionIndex++;
            }
            //check for debug mode
            var debugMode = root.get_getElementsByTagName("debug");
            if(debugMode[0].textContent == "On"){
                var tempIds  = blocks[i].getElementsByTagName("id");
                ids.push( tempIds[0].textContent + ".DebugMeter" )
                gIsDebugMeter[optionIndex] = true;
                optionIndex++;
            }
        }
