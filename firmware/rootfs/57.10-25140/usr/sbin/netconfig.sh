...
FASTCONNECTING=/var/run/fastconnecting
if [ -f ${FASTCONNECTING} ]; then
    rm ${FASTCONNECTING}
...
if [ "${MODE}" = "station" ] || [ "${MODE}" = "sonosnet" ]; then
    if [ -f /wifi/hostapd ]; then
        /wifi/hostapd
    fi
fi
