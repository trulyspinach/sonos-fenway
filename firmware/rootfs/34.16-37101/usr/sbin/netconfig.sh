...
case "$1" in
    sonosnet|station|open|credcheck|deauth|wfacert|wacexit)
        MODE="$1"
        WAC=0
        ;;
...
if [ "${WAC}" = "0" ]; then
    ...
    if [ -f /tmp/mdnsd.pid ]; then
        if [ "${KILLWACD}" = "1" ] && \
           [ "${MODE}" = "wacexit" ]; then sleep 5; fi
        kill `cat /tmp/mdnsd.pid`
        rm -f /tmp/mdnsd.pid
    fi
fi
...
elif [ "${MODE}" = "wacstart" ]; then
    /sbin/ifconfig ath0 0.0.0.0

    if [ -f /tmp/wacd.pid ]; then kill `cat /tmp/wacd.pid`; fi
    if [ -f /tmp/mdnsd.pid ]; then kill `cat /tmp/mdnsd.pid`; fi
...
if ( [ "${MODEL}" = "9" ] || [ "${MODEL}" = "14" ] ) && ( [ "${MODE}" = "sonosnet" ] || [ "${MODE}" = "station" ] ); then
